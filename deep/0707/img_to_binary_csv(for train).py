...
imgs=[]
for al in als:
    img_color = cv.imread('dataset/train/'+al+'.png', cv.IMREAD_COLOR)
...
idx = 0
csv_f = open('al.csv', 'w', encoding='utf-8')
for x in imgs:
    fdata = x.ravel()
...
for jj in range(2,8):
    imgs=[]
    for al in als:
        img_color = cv.imread('dataset/train/'+al+str(jj)+'.png', cv.IMREAD_COLOR)
        img_gray = cv.cvtColor(img_color, cv.COLOR_BGR2GRAY)
        ret, img_binary = cv.threshold(img_gray, 150, 255, cv.THRESH_BINARY)
        imgs.append(img_binary)
    ...
    idx = 0
    csv_f = open('al'+str(jj)+'.csv', 'w', encoding='utf-8')
    for x in imgs:
        fdata = x.ravel()
        img_data = list(map(lambda n: str(n), fdata))
        csv_f.write(als[idx] + ',')
